...
<body>
	<div id="sidebar" onclick="togglemenu()">
		...
		<center><br><h1><b>AVAILABLE JOBS</b></h1>
			<table border="2" cellspacing="0" style="text-align: center;align-content: center;height: 50px; width: 100%;">
				...
				<?php
					...
					if($con)
					{
						$email=$_SESSION['email'];
							$query_Stu=mysqli_query($con,"select * from `student` where `email`='$email'");
							$row=mysqli_fetch_array($query_Stu);
							$enroll_no=$row['enroll_no'];
							$sqljob="SELECT * FROM job where `job_id` not in(select `job_id` from stu_job where `enroll_no`='$enroll_no');";
							$queryjob=mysqli_query($con,$sqljob);
							while ($rowjob=mysqli_fetch_array($queryjob))
							{
								$jid=$rowjob['job_id'];
								$sql="SELECT * FROM company where `c_id` in (select `c_id` from `job` where `job_id`='$jid');";
				...
				<tr>
					...
					<th><a href="details_job.php?id=<?php echo $rowjob['job_id'];?>&cn=<?php echo $row['company_name']; ?>"><input type="button" name="details" value="DETAILS"></a></th>
					<th><a href="applyforjob.php?id=<?php echo $rowjob['job_id']; ?>"><input type="button" name="apply" value="APPLY NOW"></a></th>
				</tr>
							<?php } ?>
				<tr>
